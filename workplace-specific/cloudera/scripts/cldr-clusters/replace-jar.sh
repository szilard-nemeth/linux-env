...
if [ $# -ne 3 ]; then
    ...
    echo "Example: replace-jar.sh '*yarn*resourcemanager*jar' snemeth-sparkyarn2-1.gce.cloudera.com $HOME/development/cloudera/hadoop/"
    exit 1
fi
...
scp -i ~/.ssh/nightly_id_rsa -q -o "StrictHostKeyChecking no" ${SRC_JAR_PATH} ${CLOUDERA_HOSTNAME}:/${NEW_JAR_PATH}

echo "Running replace-jar-remote.sh on $CLOUDERA_HOSTNAME..."
ssh -i ~/.ssh/nightly_id_rsa -o "StrictHostKeyChecking no" ${CLOUDERA_HOSTNAME} \
