...
class PatchSaver:
    ...
    def run(self):
        ...
        if curr_branch == self.base_branch:
            raise ValueError(
                "Cannot make patch, current branch is {}. Please use a different branch!".format(self.base_branch)
            )
        ...
        if not result:
            raise ValueError("Patch does not apply to {}! Patch file: {}".format(self.base_branch, new_patch_filename))
        else:
            LOG.info("Patch file applies cleanly to %s. Patch file: %s", self.base_branch, new_patch_filename)
